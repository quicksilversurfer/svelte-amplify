<script>
  // Navigation data stored as reactive state for maintainability 
  ...
  // Extract content as reactive state
  const content = $state({
  ...
  <div class="mx-auto max-w-7xl px-6 py-16 sm:py-24 lg:px-8 lg:py-32">
    ...
    <div class="mx-auto max-w-2xl text-center">
      <hgroup>
        ...
        <p class="mt-2 text-4xl font-semibold tracking-tight text-balance text-gray-900 sm:text-5xl">{content.title}</p>
      </hgroup>
      <p class="mx-auto mt-6 max-w-xl text-lg/8 text-pretty text-gray-600">{content.description}</p>
      <div class="mt-8 flex justify-center">
        <a href="#" class="rounded-md bg-indigo-600 px-3.5 py-2.5 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">{content.cta}</a>
      </div>
    </div>
    ...
    <div class="mt-12 border-t border-gray-900/10 pt-8 md:flex md:items-center md:justify-between">
      ...
      <p class="mt-8 text-sm/6 text-gray-600 md:order-1 md:mt-0">&copy; {content.copyright}</p>
    </div>
  </div>
</footer>
